...
function listToFraktalMap(list: TokenList): TokenAddressMap {
  const result = listCache?.get(list)
  if (result) return result
...
// get all the tokens from Fraktal list
export function useFraktalList(): TokenAddressMap {
  const fraktalListMap = useMemo(() => listToFraktalMap(FRAKTAL_LIST), [])

  //const activeListUrls = useActiveListUrls()
  //This function must return a TokenAddressMap that can be got from my possibly, custom wrapper may be needed.

  return useMemo(() => combineMaps(fraktalListMap, FRAKTAL_LIST), [])
}

export function getFirstFraktalToken() {
  return FRAKTAL_LIST.tokens[0].address
}
...
export function useUnsupportedTokenList(): TokenAddressMap {
  ...
  //This may need its own reader that generates some stuff
